...
*/
    ...
    public:
        ...
        {
            T sub_value = this->m_unpack(event);
            auto it(this->m_callmap.find(sub_value));
            if (it != this->m_callmap.end()) {
        ...
        {
            this->m_callmap[sub_value] = callback;
        }
    ...
    public:
                ...
                , m_mouse_released_manager([](const sf::Event& event) {return event.mouseButton.button; })
        {
            if (use_builtin_helpers) {
                ...
                this->addEventCallback(sf::Event::EventType::KeyPressed, [&](const sf::Event& event) {this->m_key_pressed_manager.processEvent(event); });
                this->addEventCallback(sf::Event::EventType::KeyReleased, [&](const sf::Event& event) {this->m_key_released_manager.processEvent(event); });
                this->addEventCallback(sf::Event::EventType::MouseButtonPressed, [&](const sf::Event& event) {this->m_mouse_pressed_manager.processEvent(event); });
                this->addEventCallback(sf::Event::EventType::MouseButtonReleased, [&](const sf::Event& event) {this->m_mouse_released_manager.processEvent(event); });
            }
        }
        ...
        void removeCallback(sf::Event::EventType type)
        {
            ...
            auto it(this->m_events_callmap.find(type));
            if (it != this->m_events_callmap.end()) {
                // Remove its associated callback
                this->m_events_callmap.erase(it);
            }
        }
    ...
    public:
        ...
        void processEvents(EventCallback fallback = nullptr) const
        {
            ...
            while (this->m_window.pollEvent(event)) {
                this->m_event_map.executeCallback(event, fallback);
            }
        }
        ...
        void addEventCallback(sf::Event::EventType type, EventCallback callback)
        {
            this->m_event_map.addEventCallback(type, callback);
        }

        // Removes a callback
        void removeCallback(sf::Event::EventType type)
        {
            this->m_event_map.removeCallback(type);
        }

        // Adds a key pressed callback
        void addKeyPressedCallback(sf::Keyboard::Key key, EventCallback callback)
        {
            this->m_event_map.addKeyPressedCallback(key, callback);
        }

        // Adds a key released callback
        void addKeyReleasedCallback(sf::Keyboard::Key key, EventCallback callback)
        {
            this->m_event_map.addKeyReleasedCallback(key, callback);
        }

        // Adds a mouse pressed callback
        void addMousePressedCallback(sf::Mouse::Button button, EventCallback callback)
        {
            this->m_event_map.addMousePressedCallback(button, callback);
        }

        // Adds a mouse released callback
        void addMouseReleasedCallback(sf::Mouse::Button button, EventCallback callback)
        {
            this->m_event_map.addMouseReleasedCallback(button, callback);
        }
        ...
        {
            const sf::Vector2i mouse_position = sf::Mouse::getPosition(this->m_window);
